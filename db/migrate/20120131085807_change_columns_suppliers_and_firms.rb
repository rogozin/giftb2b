...
#    add_column :firms, :supplier_id, :integer    
    ...
    Supplier.all.each do |sup|
      ...
      firm = Firm.create(:name => sup.name, :supplier_id => sup.id, :addr_f => sup.address, :description => sup.terms, :email => email, :phone => "+7(495)")
      
      u = User.new({:username => sup.permalink, :password=>pass, :password_confirmation => pass, :email => email, :company_name => sup.name, :city => "Москва",  :phone => "+7(495)", :active => true, :fio => sup.name}, :as => :admin)
      u.firm = firm
      u.role_objects = [lk_sup_role, supplier_role]
      if u.save
        puts "================================================================="
