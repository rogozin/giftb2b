...
class SampleNotificationJob
 attr_reader :logger
  ...
  def perform
      ...
      client.query("select id, supplier_return_date from #{db["database"]}.samples where closed=0 and responsible_id > 0 and (supplier_return_date = curdate()  or supplier_return_date  = adddate(curdate(),2) )").each do |row|
      ...
      client.query("select id, client_return_date from #{db["database"]}.samples where closed=0 and responsible_id > 0 and  (client_return_date = curdate()  or supplier_return_date  = adddate(curdate(),2))").each do |row|
