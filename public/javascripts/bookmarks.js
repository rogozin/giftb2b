				...
				if(obj!=lasthash){
					if(lasthash!=undefined){
						...
						if(phas && purl && pctn){
							lasthash = phas;
							on_complete = ''
							if (purl.indexOf('categories')==1) {on_complete = 'catalog_load_complete();'}
							if (purl.indexOf('products')==1) {on_complete = 'product_load_complete();'}							
							ajax.load(purl,pctn,false,'',[on_complete]);
						}
					}
				}
			}
